...
const RESULT_NOT_FOUND =
  'Sorry. I was unable to track down what you were looking for.';
const UNAUTHORIZED =
  'Using bearer auth. Plugin service level key was either not provided or incorrect.';
...
server.get('/.well-known/ai-plugin.json', (req, res) => {
  ...
  res.json({
    ...
    description_for_human:
      "Search the employee directory with a natural language description of what you're looking for.",
    description_for_model:
      'Help the user query the company employee directory.',
    auth: {
      type: 'service_http',
...
server.post('/search', authMiddleware, async (req, res) => {
  ...
  if (typeof req.body.message === 'undefined' || !req.body.message.length) {
    res
      .status(400)
      .json({ error: 'No query provided in "message" key of payload.' });
    return;
  }
  try {
